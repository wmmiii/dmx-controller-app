...
fn select(
    ...
    // Hold pins a single item, so there is never a next to cross into.
...
///
/// Returns an error if `order_kind` is unknown or the playlist timing is unset.
pub fn active_playlist_selection(
    ...
    // Both indices are `< len`, and `len` came in as a `u32`, so the cast back
    // is lossless.
...
fn resolve_pattern_order(playlist: &Playlist) -> Result<ResolvedOrder, String> {
    ...
    {
        playlist::PatternOrder::PatternHold(Hold { id }) => Ok(ResolvedOrder::Hold(hold_index(
            playlist.patterns.iter().position(|p| p.id == *id),
        )?)),
...
fn resolve_palette_order(playlist: &Playlist) -> Result<ResolvedOrder, String> {
    ...
    {
        playlist::PaletteOrder::PaletteHold(Hold { id }) => Ok(ResolvedOrder::Hold(hold_index(
            playlist.palettes.iter().position(|p| p.id == *id),
        )?)),
...
fn hold_index(index: Option<usize>) -> Result<usize, String> {
    index.ok_or_else(|| "Held playlist item not found".to_string())
}
...
// splitmix64 finalizer — deterministic and well-distributed.
