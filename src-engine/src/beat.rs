...
pub const MAX_BEAT_SAMPLES: usize = 16;
...
const STALE_SAMPLE_FACTOR: f64 = 1.8;
...
#[must_use]
#[allow(clippy::cast_precision_loss)]
pub fn beat_t(project: &Project, t: u64) -> Option<f64> {
    let beat = effective_beat_metadata(project, t)?;
    if beat.length_ms <= 0.0 {
        return None;
    }
    let elapsed = t as f64 - beat.offset_ms as f64;
    let raw = (elapsed / beat.length_ms) % 1.0;
    // Modulo of a negative elapsed time can produce a negative result.
    Some(if raw < 0.0 { raw + 1.0 } else { raw })
}
...
pub fn transition_beat(project: &mut Project, next_beat: &BeatMetadata, t: u64) {
    let Some(prev) = effective_beat_metadata(project, t) else {
    ...
    let mut adjusted_beat = *next_beat;
    if prev.length_ms > 0.0 && adjusted_beat.length_ms > 0.0 {
        let prev_beat_count = ((t as f64 - prev.offset_ms as f64) / prev.length_ms).floor();
        let next_beat_count =
            ((t as f64 - adjusted_beat.offset_ms as f64) / adjusted_beat.length_ms).floor();
        let drift = (next_beat_count - prev_beat_count).round() as i64;
        adjusted_beat.offset_ms = (adjusted_beat.offset_ms as f64
            + drift as f64 * adjusted_beat.length_ms)
            .round()
            .max(0.0) as u64;
    }

    project.prev_live_beat = Some(prev);
    project.live_beat = Some(adjusted_beat);
    project.beat_transition_start_ms = t;
    project.beat_transition_duration_ms = adjusted_beat.length_ms as u64;
}
