 ...
 * currently active. Call once per subsystem with that subsystem's order case,
 * held-item index, and collection length. `currentIndex`/`nextIndex` index into
 * the caller's own array.
 *
 * Returns null if WASM isn't loaded yet, the order case is unset/unknown, or the
 * playlist timing isn't set.
 */
