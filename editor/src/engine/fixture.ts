...
function getPhysicalWritableDevice(
  ...
  const physicalFixture =
    getActiveUniverse(project).fixtures[fixtureId.toString()];
  const definition =
  ...
  if (groupId === GROUP_ALL_ID) {
    Object.values(getActiveUniverse(project).fixtures)
      ...
      .forEach((f) =>
        collectFunctions(
          f,
          project.fixtureDefinitions[f.fixtureDefinitionId].modes[
            f.fixtureMode
          ],
          functionCollection,
        ),
    ...
    getAllFixtures(project, groupId).forEach((id) => {
      const physicalFixture =
        getActiveUniverse(project).fixtures[id.toString()];
      const definition =
        project.fixtureDefinitions[physicalFixture.fixtureDefinitionId];
      const mode = definition.modes[physicalFixture.fixtureMode];
      collectFunctions(physicalFixture, mode, functionCollection);
...
export function getAvailableChannels(
  ...
  const channels = new Set(
    fixtureIds
      ...
      .map(
        (pf) =>
          project.fixtureDefinitions[pf.fixtureDefinitionId]?.modes[
            pf.fixtureMode
          ],
      )
      .filter((m) => m != null)
...
export function deleteFixture(project: Project, fixtureId: bigint) {
  ...
  for (const group of Object.values(project.groups)) {
    ...
    if (fixtureList) {
      fixtureList.fixtures = fixtureList.fixtures.filter(
        (f) => f !== fixtureId,
      );
    }
  }
  ...
  const deleteFromLightTrack = (t: LightTrack) => {
    ...
    if (t.outputId.output.case === 'fixtures') {
      if (
        ...
        delete t.outputId.output.value.fixtures[
...
interface FunctionCollection {
  manualFunctions: Array<
    (universe: DmxUniverse, index: number, value: number) => void
  >;
  colorFunctions: Array<
    (universe: DmxUniverse, r: number, g: number, b: number, w: number) => void
  >;
  angleFunctions: Map<
    AngleChannel,
    Array<(universe: DmxUniverse, a: number) => void>
  >;
  amountFunctions: Map<
    AmountChannel,
    Array<(universe: DmxUniverse, a: number) => void>
...
export function deleteFixtureGroup(project: Project, fixtureGroupId: bigint) {
  ...
  // Delete from groups.
  for (const group of Object.values(project.groups)) {
    group.groups = group.groups.filter((g) => g !== fixtureGroupId);
  }
