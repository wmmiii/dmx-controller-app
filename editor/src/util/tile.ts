...
export function tileActiveAmount(tile: Scene_Tile, beat: BeatMetadata, t: bigint): number {
...
export function toggleTile(tile: Scene_Tile, beat: BeatMetadata) {
  const enabled = tile.oneShot ||
    tile.transition.case === 'startFadeOutMs' ||
    (tile.transition.case === 'absoluteStrength' && tile.transition.value < 0.1);

  const t = BigInt(new Date().getTime());
  if (tile.transition.case === undefined || tile.transition.case === 'absoluteStrength') {
    tile.transition = {
      case: 'startFadeOutMs',
      value: 0n,
    };
